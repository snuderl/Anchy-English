...
class TestWorksheetEndpoints:
    ...
    def test_get_single_worksheet(self, client, sample_worksheet):
        ...
        response = client.get(f"/api/worksheets/{sample_worksheet.id}")
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_get_nonexistent_worksheet(self, client):
        ...
        response = client.get("/api/worksheets/9999")
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_create_worksheet(self, client, db_session):
        ...
        worksheet = db.session.get(Worksheet, data["id"])
    ...
    def test_update_worksheet(self, client, sample_worksheet):
        ...
        response = client.post(
            f"/api/worksheets/{sample_worksheet.id}",
    ...
    def test_delete_worksheet(self, client, sample_worksheet):
        ...
        response = client.get(f"/api/worksheets/{worksheet_id}/delete")
        assert response.status_code == 200

        deleted = db.session.get(Worksheet, worksheet_id)
...
class TestCategoryEndpoints:
    ...
    def test_create_category_with_parent(self, client, sample_category):
        ...
        category_data = {"name": "Child Category", "parent": sample_category.name}
    ...
    def test_delete_category(self, client, sample_category):
        ...
        response = client.post(f"/api/categories/{category_id}/delete")
        assert response.status_code == 200

        deleted = db.session.get(Category, category_id)
        assert deleted is None
...
class TestWordEndpoints:
    ...
    def test_words_are_unique(self, client, db_session):
        ...
        same_count = sum(
            1 for w in data if w["english"] == "same" and w["slovene"] == "isto"
        )
        assert same_count == 1
