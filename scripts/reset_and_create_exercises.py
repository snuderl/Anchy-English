...
def reset_and_create_exercises():
    with app.app_context():
        # Clear all relationships first, then delete
        ...
        # Now delete all exercises and exercise sets
        ...
        if not daily_life_cat:
            daily_life_cat = Category(name="Daily Life")
            db.session.add(daily_life_cat)

        family_cat = Category.query.filter_by(name="Family").first()
        if not family_cat:
            family_cat = Category(name="Family")
            db.session.add(family_cat)

        travel_cat = Category.query.filter_by(name="Travel").first()
        if not travel_cat:
            travel_cat = Category(name="Travel")
            db.session.add(travel_cat)
