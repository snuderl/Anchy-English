...
from app.baza import db, Worksheet, Translation, Category, CategoryToWorksheet
...
def seed():
    with app.app_context():
        ...
        for ws_data in WORKSHEETS:
            ...
            if existing:
                print(f"  Skipping '{ws_data['ime']}' (already exists with {len(existing.translations)} words)")
                continue
            ...
            db.session.add(ws)
            print(f"  Created '{ws_data['ime']}' with {len(ws_data['words'])} words in '{ws_data['category']}'")
